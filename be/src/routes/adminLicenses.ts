...
adminLicensesRouter.post('/', async (req, res, next) => {
  try {
    ...
    const created: { licenseKey: string; durationDays: number | null; maxDevices: number }[] = []
    ...
    await prisma.$transaction(async (tx) => {
      for (let i = 0; i < quantity; i += 1) {
        ...
        let tries = 0
        while (tries < 20) {
          const exists = await tx.license.findUnique({ where: { licenseKeyHash: hash } })
          if (!exists) break
        ...
        created.push({ licenseKey: plain, durationDays: durationDays ?? null, maxDevices })
      }
    })

    await audit(actorId, 'admin.license.create', 'license', 'bulk', { quantity })
...
adminLicensesRouter.delete('/:licenseId/devices/:activationId', async (req, res, next) => {
  try {
    ...
    if (!activation) {
      return res.status(404).json({ success: false, code: 'NOT_FOUND', message: 'Activation not found.' })
    }
    ...
    await audit(actorId, 'admin.license.revoke_device', 'license_activation', activationId, { licenseId })
...
adminLicensesRouter.get('/:id', async (req, res, next) => {
  try {
    ...
    if (!lic) {
      return res.status(404).json({ success: false, code: 'NOT_FOUND', message: 'License not found.' })
    }
...
adminLicensesRouter.patch('/:id/extend', async (req, res, next) => {
  try {
    ...
    if (!parsed.success) {
      return res.status(400).json({ success: false, code: 'INVALID_PAYLOAD', message: 'Invalid extend payload.' })
    }
